...
The class src/SprykerCommunity/Client/ErpIntegration/Models/BaseRequestBuilder provides a request builder, logging and handling of failed responses.
You have to adjust the following places:
- `BaseRequestBuilder::DEFAULT_REQUEST_TIMEOUT_SECONDS` - with the default timeout for requests.
- `BaseRequestBuilder::getRequestOptions` - request parameters, including authentication, referrer. See [Guzzle documentation](https://docs.guzzlephp.org/en/stable/request-options.html) for a full list of options.
- `BaseRequestBuilder::buildRequestHeaders` - request headers, i.e. content type, accept, and anything 3rd party specific.
...
In case of a special connection setup for [Guzzle client configuration](https://docs.guzzlephp.org/en/5.3/clients.html), please apply it to `ErpIntegrationDependencyProvider::addGuzzleClient`.

### Validate logging and error handling.

Other methods follow Spryker best practices and provide detailed logs in each case:
- `BaseRequestBuilder::handleFailedConnectionResponse` - when connection failed, usually due to timeout or server error. Request is logged at error level.
- `BaseRequestBuilder::handleGenericRequestFailureResponse` - any non-connection related failure. Request is logged at error level.
- `BaseRequestBuilder::createFailedResponse` - provides a template for generating the failed response, including the message from the exception.
- `BaseRequestBuilder::logRequest` - logging request at info level.
- `BaseRequestBuilder::logResponse` - logging response at info level.
...
The next step is request model preparation. Start with a copy of model `src/SprykerCommunity/Client/ErpIntegration/Models/ExampleRequest`, which doesn't require an interface since it contains a single method without a reason for extension.
...
To implement mapping of the request and response, create a copy of `src/SprykerCommunity/Client/ErpIntegration/Models/ExampleRequestMapper` and adjust:
